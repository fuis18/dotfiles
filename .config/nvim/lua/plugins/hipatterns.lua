local css_colors = {}
for name, hex in ([[aliceblue=F0F8FF
antiquewhite=FAEBD7
aqua=00FFFF
aquamarine=7FFFD4
azure=F0FFFF
...
whitesmoke=F5F5F5
yellow=FFFF00
yellowgreen=9ACD32]]):gmatch("(%w+)=(%x+)") do
  css_colors[name] = hex
end
...
return {
  {
    ...
    opts = function()
      ...
      local gen = hipatterns.gen or hipatterns.gen_highlighter
      local function is_stylesheet(buf)
      ...
      local function stylesheet_pattern(pattern)
        return function(buf)
          if is_stylesheet(buf) then
            return pattern
          end
        end
      end
